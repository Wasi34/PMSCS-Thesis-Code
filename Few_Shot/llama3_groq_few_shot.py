"""
llama3_groq_few_shot.py
-----------------------
Few-Shot Question Answering on Bengali SQuAD using LLaMA 3 via Groq API.
...
Usage:
    python few_shot/llama3_groq_few_shot.py --rows 10

Output:
    outputs/llama3_groq/few_shot_results.json
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Few-Shot QA using LLaMA 3 (Groq) on Bengali SQuAD"
    )
    parser.add_argument(
        "--rows",
        type=int,
...
def query_llama(client: Groq, context: str, question: str) -> dict:
    """Send a few-shot prompt to LLaMA 3 via Groq and return parsed JSON response."""
...
def main():
    ...
    print(f"\nRunning Few-Shot QA with {MODEL} (via Groq) on {len(df)} rows...\n")
